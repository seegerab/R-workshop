...
colnames(data)[colnames(data) == "X...record_id"] <- "record_id"
```
...
Next, create a subset of just people living with HIV. 
...
data <- data%>%
  ...
  mutate(hiv_status = case_when(age_cat == "15 and older" & bf_hivstatus == "" ~ "Positive",
...
```{r}
table(data$hiv_status, data$bf_hivstatus, exclude = NULL)%>%addmargins()
```
