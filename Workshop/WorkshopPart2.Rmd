...
The **null deviance** is the amount of variability that is explained by the **null model** - no predictors, just the model intercept. 

The **residual deviance** is the amount of variability that the full model explains. 
...
```{r}
### Filter to all participants who initiated on ATT
att_init_ids <- data%>%
...
In addition, create a variable for the outcome. Once again, this is not found in the **baseline** form.
...
We can investigate what is going on with the particiapnts who do not have a trace stool Xpert Ultra result yet did not start treatment.
...
This may be something to revisit and confirm with Miguel - it appears that 18 participants do not have an outcome reported.
