...
**Very small group size**: With just 2 inpatient participants, the model has very little adult_subset to reliably estimate the effect of being inpatient. Further, both of them has a result that is "Trace".
...
What does this mean? For continuous predictors, a one-unit increase should correspond to a consistent change in the log odds of the outcome.
...
Among those who have a positive stool Xpert Ultra result, who initiated TB treatment?

First, who initiated ATT? 
...
If we use the `adult_subset`, it appears that we do not have information on whether 33 adults started ATT.

However, this is because the `adult_subset` is filtering to just the ****
