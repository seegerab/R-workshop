...
We are interested in the stool Xpert Ultra trace results. First, lets filter to all participants who have a positive stool Xpert Ultra result. And, create a binary variable with whether or not the stool Xpert Ultra result is trace or not. 
...
```{r}
### We did these same steps in the first .Rmd
data <- data%>%
...
For example, consider the relationship between `bf_weight`, `bf_height`, and our newly created variable `bmi`.

We can use the `ggpairs()` function form the `GGally` package to get useful information about the relationship between variables. This function provides a scatterplot and correlations for each pair, as well as density plots for each variable.
...
library(GGally)
ggpairs(data%>%select(bf_weight, bf_height, bmi))
```

Notice that height and weight are highly correlated, as well as BMI and weight. 

It wouldn't make sense to include all three variables in the model. Rather, we will just include BMI. 
...
100% of adults are living with HIV. This means there's no variation in HIV status among adults — their HIV status is completely determined by age. This is known as **perfect separation** or **perfect collinearity** between age and HIV status.
...
In light of what we've investigated so far, we have a few options moving forward. 

## 1. Just adults.

This would allow us to include CD4 in the model.
...
57 / 59 
```
This variable is mostly complete; just 2 adults living with HIV are missing CD4 count. 
...
However, only 2 participants are living with HIV, and both have a stool Xpert Ultra trace result. This will lead to model fit issues.

## 3. Include both age categories

If we include all ages, we won't be able to include HIV status because of the perfect separation we identified above.
...
For the workshop, we'll continue with this analytic subgroup. However, what should we do differently for the manuscript?
...
```{r}
table(data$rf_enrollocation, data$stool_semiquant_bin, exclude = NULL)%>%
  addmargins()
```

```{r}
ggplot(data = data, aes(x = rf_enrollocation, fill = stool_semiquant_bin))+
  geom_bar()+
...
**Very small group size**: With just 3 inpatient participants, the model has very little data to reliably estimate the effect of being inpatient.
...
```{r}
table(data$rf_gender, data$stool_semiquant_bin, exclude = NULL) %>%
  addmargins()
```
...
### Build the model!
logistic_regression_model <- glm(outcome ~ rf_gender, data = data, family = "binomial")
```
...
```{r}
final_model <- glm(outcome ~ rf_country + rf_gender + bmi + cd4_cat, data = adult_subset, family = "binomial")
```
