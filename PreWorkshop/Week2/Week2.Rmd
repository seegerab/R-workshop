...
Often, we want to create 2x2 contingency tables to examine the relationship between two categorical variables. For example, we might want a breakdown of paritipant enrollment location by site. 
...
```{r}
table(data$rf_enrollocation, data$rf_country, exclude = NULL)%>%
  addmargins()
```

There are X total participants in this table. X enrolled in Eswatini, Y in Tanzania, and Z in Mozambique.

Of the Z participants enrolled in Mozambique, ZZ were enrolled in an Outpatient setting (ZZ/Z = ZZ%). This is an example of a conditional probability - it is the chance that someone is enrolled in an outpatient setting, **given** we know that they were enrolled in Mozambique.
...
For example, let's find the minimum weight by country:

```{r}
### Use the pipe operator (%>%) from the tidyverse package
data%>%
  ### Group by the categorical variable - in this case, country
  group_by(rf_country)%>%
